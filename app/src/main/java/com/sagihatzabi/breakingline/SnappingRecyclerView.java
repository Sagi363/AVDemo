...
public class SnappingRecyclerView extends RecyclerView {
	private static final float SCALE_SIZE = 0.77f;
	private static final float ALPHA_SIZE = 0.75f;
	...
	private void updateViews() {
		for (int i = 0; i < getChildCount(); i++) {
			...
			if (_scaleViews) {
				float scale = 1f - (SCALE_SIZE * percentage);

				child.setScaleX(scale);
				child.setScaleY(scale);

				if (_alphaViews) {
					float opacity = 1f - (ALPHA_SIZE * percentage);
